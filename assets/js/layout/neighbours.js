...
function _inspect(φ) {
  return `${φ.title} — rowStart: ${φ.rowStart}, colStart: ${φ.colStart}, rowSpan: ${φ.rowSpan}, colSpan: ${φ.colSpan}`
}
...
function _horizontalOverlap(φ, ϑ) {
  let overlapAbove  = ϑ.rowStart <= φ.rowStart && φ.rowStart < (ϑ.rowStart + ϑ.rowSpan),
      overlapBelow  = φ.rowStart <= ϑ.rowStart && (φ.rowStart + φ.rowSpan) > ϑ.rowStart
  return overlapAbove || overlapBelow
}

function _verticalOverlap(φ, ϑ) {
  let overlapLeft  = ϑ.colStart <= φ.colStart && φ.colStart < (ϑ.colStart + ϑ.colSpan),
      overlapRight = φ.colStart <= ϑ.colStart && (φ.colStart + φ.colSpan) > ϑ.colStart
...
function above(φ, Φ) {
  ...
  let ꜰℕ = (φ, ϑ) => { 
    ...
    let overlap = _verticalOverlap(φ, ϑ)                    // do the items overlap horizontally?
...
function right(φ, Φ) {
  let ꜰℕ = (φ, ϑ) => { 
    ...
    let εφ      = φ.itemExtent,
        εϑ      = ϑ.itemExtent,
        overlap = _horizontalOverlap(φ, ϑ)
    if(!overlap) return null     
